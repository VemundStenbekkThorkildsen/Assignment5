...
This needs to be equal to a constant $\lambda$. Because of the given boundaries, $G(y)$ has to equal $sin(\sqrt{\lambda}y)$.\\

And our boundaries $G(0)=0$ and $G(H=1)=0$ gives us $\lambda=(n\pi)^2$\\
\vspace{0.5cm}
Now we look at $F_{xx}=\lambda F$\\
We make a new ansatz: $F(x)=Ae^{(n\pi x)}+Be^{-(n\pi x)}$
Our boundary $F(0)=0$ gives us $B=-A$\\
\vspace{0.5cm}
which gives us our expression for $F(x)$
$\rightarrow F(x)=Asinh(n\pi x)$
\vspace{0.5cm}
Giving our solution $u(x,y)=F(x)G(x)$:\\
\begin{equation}
u(x,y)=\sum^\infty_{n=1}A_nsinh(n\pi x)sin(n\pi y)
\end{equation}
we then use our last boundary $u(1,y)=1$ to find an expression for $A_n$ by taking the integral of both sides: $$\int_0^1 1*sin(m\pi y) dy = \int_0^1 \sum^\infty_{n=1}A_nsinh(n\pi 1)sin(n\pi y)*sin(m\pi y)$$\\
...
Giving us an equation for $A_n$:\\
$$\frac{2}{sinh(n\pi)}\int_0^1sin(n\pi y) dy$$
The integral in the equation has solutions:
odd $n = \frac{2}{n\pi}$, even $n = 0$ we can now solve the problem analytic with ein n så bare får det egentlig fra 0-evig men me har gjort det me n fra 0-100 cuz its fine my nigga\\
